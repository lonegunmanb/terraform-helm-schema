...
import (
	...
	"github.com/lonegunmanb/terraform-helm-schema/v3/schema"
	"golang.org/x/oauth2"
)

func main() {
	refreshFlag := flag.Bool("refresh", false, "Refresh Helm schema")
...
func checkGitHubTag(schemaVersion string) (bool, error) {
	...
	for {
		tags, resp, err := client.Repositories.ListTags(context.Background(), "lonegunmanb", "terraform-helm-schema", options)
		if err != nil {
			return false, err
		}
